...
#define INIT_LENGTH_INPUT_BUFFER 128
...
static struct Info_Input_Internal
{
	struct Info_Input info;
	struct Point pos;
};

static struct Info_Input_Internal *input_buffer;
static int length_input_buffer;
...
void Start_Console(void)
{
	...
	input_buffer = malloc(sizeof(struct Info_Input_Internal) * INIT_LENGTH_INPUT_BUFFER);
	length_input_buffer = INIT_LENGTH_INPUT_BUFFER;
...
double Stopwatch_Get()
{
	...
	else
	{
		...
		result = now.QuadPart - clock_startTime.QuadPart;
...
static struct Info_Input_Internal *AcceptAndGet_Next_Ptr_Input()
{
	++count_accepted_inputs;

	if ( count_accepted_inputs == length_input_buffer )
	{
		int idx;

		struct Info_Input_Internal *new_buffer = malloc(sizeof(struct Info_Input_Internal) * length_input_buffer * 2);

		for ( idx = 0; idx < length_input_buffer; ++idx )
			new_buffer[idx] = input_buffer[idx];

		free(input_buffer);

		input_buffer = new_buffer;
		length_input_buffer *= 2;
	}

	return &input_buffer[count_accepted_inputs];
}
		...
		case MOUSE_EVENT:
			...
			if ( last_state_mouse_button != 0 )
			{
				...
				while ( state_released != 0 )
				{
					...
					ptr_input_buffer = AcceptAndGet_Next_Ptr_Input();
